...
public class GeneradorJustificantePago {
	...
	public static byte[] generarJustificantePago(final String dirConf, final DatosSesionPago datosPago) {
		...
		if (!f.exists()) {
			throw new NoExistePlantillaJustificanteEntidadException(datosPago.getEntidadId(), pathFichero);
		}
		...
		try {
			...
			throw new NoExistePlantillaJustificanteEntidadException(datosPago.getEntidadId(), pathFichero, ioe);
		}
